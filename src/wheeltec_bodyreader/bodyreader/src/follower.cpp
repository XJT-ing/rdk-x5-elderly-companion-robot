...
float target_distance = 1400;
float x_p = 0;
float x_d = 0;
float z_p = 0;
float z_d = 0;
float z_max = 1.0;
float z_accel_max = 0.3;
float angle_deadzone = 0.01;
...
void bodyposture_Callback(bodyreader_msg::msg::Bodyposture msg)
{
	if (mode == 2)
	{
		...
		if(error_x_angle > -angle_deadzone && error_x_angle < angle_deadzone)  error_x_angle = 0;
		...
		if(cmd_vel_msg.angular.z < -z_max)  cmd_vel_msg.angular.z = -z_max;
		else if(cmd_vel_msg.angular.z > z_max) cmd_vel_msg.angular.z = z_max;

		static float last_angular_z = 0;
		float delta_z = cmd_vel_msg.angular.z - last_angular_z;
		if(delta_z > z_accel_max)        cmd_vel_msg.angular.z = last_angular_z + z_accel_max;
		else if(delta_z < -z_accel_max)  cmd_vel_msg.angular.z = last_angular_z - z_accel_max;
		last_angular_z = cmd_vel_msg.angular.z;
...
int main(int argc, char *argv[])
{
    ...
    node->declare_parameter<float>("bodyfollow_z_p", 0.01);
  	...
  	node->declare_parameter<float>("bodyfollow_target_x_angle", 0.0);
  	node->declare_parameter<float>("bodyfollow_target_distance", 1400.0);
  	node->declare_parameter<float>("bodyfollow_z_max", 1.0);
  	node->declare_parameter<float>("bodyfollow_z_accel_max", 0.3);
  	node->declare_parameter<float>("bodyfollow_angle_deadzone", 0.01);
  	...
  	node->get_parameter("bodyfollow_target_x_angle", target_x_angle);
  	node->get_parameter("bodyfollow_target_distance", target_distance);
  	node->get_parameter("bodyfollow_z_max", z_max);
  	node->get_parameter("bodyfollow_z_accel_max", z_accel_max);
  	node->get_parameter("bodyfollow_angle_deadzone", angle_deadzone);
