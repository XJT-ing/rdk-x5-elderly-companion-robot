...
# ═══════════════════════════════════════════════════════
# 配置区：修改下面这个变量即可，不需要改逻辑代码
#   True  = 跟随时有提示音
#   False = 静默跟随
# ═══════════════════════════════════════════════════════
VOICE_FEEDBACK_ON_FOLLOW = False
...
class SoundBodyFollower(Node):
        ...
        while rclpy.ok():
            ...
            try:
                ...
                for cmd in commands:
                    ...
                    if actuator == "底盘" and action == "人体跟踪" and params.get("状态") == "开始":
                        self._start_follow()
                    elif actuator == "底盘" and action == "人体跟踪" and params.get("状态") == "停止":
                        self._stop_follow()
    ...
    def _start_follow(self):
        ...
        with self._lock:
            ...
            # 语音反馈：由配置区 VOICE_FEEDBACK_ON_FOLLOW 控制
            if VOICE_FEEDBACK_ON_FOLLOW:
                subprocess.Popen(
                    ["ros2", "param", "set", "/body_feedback", "voice_feedback", "true"],
                    stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL,
                )
            else:
                self.get_logger().info("（静默跟随，语音已禁用）")
            msg = Int8()
            msg.data = 2
            self._mode_pub.publish(msg)
